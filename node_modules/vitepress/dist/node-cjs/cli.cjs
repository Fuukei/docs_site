...
var serve = require('./serve-a64f9d02.cjs');
require('fs');
require('path');
require('url');
require('dns');
...
if (!command || command === "dev") {
  const createDevServer = async () => {
    const server = await serve.createServer(root, argv, async () => {
      await server.close();
  ...
  createDevServer().catch((err) => {
    console.error(serve.picocolors.exports.red(`failed to start server. error:
`), err);
    process.exit(1);
  });
...
`), err);
  ...
  console.log(serve.picocolors.exports.red(`unknown command "${command}".`));
  process.exit(1);
}
